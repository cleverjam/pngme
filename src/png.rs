...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let is_valid_header =
            iter::zip(header, Png::STANDARD_HEADER).all(|(given, standard)| given == &standard);
        ...
        while index < value.len() {
            let chunk = Chunk::try_from(&value[index..])
                .expect(&format!("Invalid chunk found at index {}\n", index,));
            index = index + chunk.length() as usize + 12;
            chunks.push(chunk);
        }
        Ok(Self { chunks })
    }
